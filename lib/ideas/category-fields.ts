...
/**
 * Validates dynamic field values for a draft payload.
 *
 * Identical to `validateDynamicFieldValues` except required-field enforcement
 * is skipped — drafts allow partial saves with empty required fields.
 */
export function validateDraftDynamicFieldValues(
  rules: CategoryFieldRule[],
  rawValues: Record<string, FormDataEntryValue | null>,
): DynamicFieldValidationResult {
  const errors: Record<string, string> = {}
  ...
  for (const rule of rules) {
    ...
    // Skip empty values — drafts do not enforce required constraints
